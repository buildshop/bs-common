    ...
    public function renderBody() {
        ...
        if ($this->showErrorSummary && ($model = $this->getModel(false)) !== null) {
            ...
            if (Yii::app()->controller instanceof AdminController) {                
                if($errorSummary) $output.=Yii::app()->tpl->alert('warning', $errorSummary,false) . "\n";
            } else {
                $output.=$errorSummary . "\n";
            }
        }
        ...
        if ($element->getVisible()) {
            if ($element instanceof FormInputElement) {
                if ($element->type === 'hidden')
                    ...
                    return "<div class=\"form-group field_{$element->name}\"><div class=\"col-sm-4\">" . $element->renderLabel() . "</div><div class=\"col-sm-8\">" . $element->renderInput() . "" . $element->renderHint() . "" . $element->renderError() . "</div></div>\n";
            }
