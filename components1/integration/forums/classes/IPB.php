...
class IPB extends GlobalClass {
    ...
    public function log_out() {
        $this->FDB->createCommand("UPDATE {$this->tbl_prefix}sessions SET member_name='', seo_name='', member_id='0', login_type='0' WHERE id='" . Yii::app()->request->cookies['session_id']->value . "'")->execute();
        ...
        $this->FDB->createCommand("UPDATE {$this->tbl_prefix}members SET last_visit='" . time() . "', last_activity='" . time() . "' WHERE member_id='" . Yii::app()->request->cookies['member_id']->value . "'")->execute();
        return true;
    }
    ...
    public function check_user($login, $user_pass) {
        $result = $this->FDB->createCommand("SELECT email, members_pass_hash, members_pass_salt FROM {$this->tbl_prefix}members WHERE LOWER(name)='" . strtolower($login) . "'");
