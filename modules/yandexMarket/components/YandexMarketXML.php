...
class YandexMarketXML {
    ...
    public function __construct() {
        $this->_config = Yii::app()->settings->get('shop');
    ...
    public function renderShopData() {
        $this->write('<name>' . $this->_config['yandexMarket_name'] . '</name>');
        $this->write('<company>' . $this->_config['yandexMarket_company'] . '</company>');
        $this->write('<url>' . $this->_config['yandexMarket_url'] . '</url>');
    }
    ...
    public function loadProducts() {
        $limit = $this->limit;
        $total = ceil(ShopProduct::model()->active()->count() / $limit);
        $offset = 0;

        $this->write('<offers>');

        for ($i = 0; $i <= $total; ++$i) {
            $products = ShopProduct::model()->active()->findAll(array(
                'limit' => $limit,
                'offset' => $offset,
                    ));
        ...
        foreach ($products as $p) {
            if (!count($p->variants)) {
                $this->renderOffer($p, array(
                    'url' => Yii::app()->createAbsoluteUrl('/shop/product/view', array('seo_alias' => $p->seo_alias)),
                    'price' => Yii::app()->currency->convert($p->price, $this->_config['yandexMarket_currency_id']),
                ...
                foreach ($p->variants as $v) {
                    ...
                    $this->renderOffer($p, array(
                        'url' => Yii::app()->createAbsoluteUrl('/shop/product/view', array('seo_alias' => $p->seo_alias)) . $hashtag,
                        'price' => Yii::app()->currency->convert(ShopProduct::calculatePrices($p, $p->variants, 0), $this->_config['yandexMarket_currency_id']),
                        'currencyId' => $this->currencyIso,
                        'categoryId' => $p->mainCategory->id,
                        'picture' => $p->mainImage ? Yii::app()->createAbsoluteUrl($p->mainImage->getUrl()) : null,
