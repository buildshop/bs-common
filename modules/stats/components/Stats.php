...
class Stats extends CApplicationComponent {
    ...
    public function visitSystem($date, $fdate = false, $rwz = false) {
        if ($date != $fdate && isset($rwz["cnt3"][$date])) {
            $m_se[$date] = $rwz["cnt3"][$date];
            $m_other[$date] = $rwz["cnt4"][$date];
            $m_fix[$date] = $rwz["cnt5"][$date];
        } else {
            ...
            if (isset($rwz["dt"])) {
                if ($date != $fdate and !in_array($date, $rwz["dt"])) {
                    mysql_query("INSERT INTO mainh(dt,cnt1,cnt2,cnt3,cnt4,cnt5) VALUES('" . $date . "','" . $m_uniqs[$date] . "','" . $m_hits[$date] . "','" . $m_se[$date] . "','" . $m_other[$date] . "','" . $m_fix[$date] . "')");
    ...
    public function countFix($date) {
        if (isset($this->_zfx)) {
            $sql = "SELECT COUNT(i) FROM cms_surf WHERE (" . $this->_zfx . ") AND dt='" . $date . "' AND " . $this->_zp . "";
            $command = Yii::app()->db->createCommand($sql);
            $res = $command->queryRow(false);
            return $res[0];
        } else {
    ...
    public function countVisits($date) {
        ...
        //  die('dasdas');
        if ($zp) {
            $sql = "SELECT COUNT(DISTINCT ip), COUNT(i) FROM cms_surf WHERE dt='" . $date . "' AND " . $zp;
            $command = Yii::app()->db->createCommand($sql);
