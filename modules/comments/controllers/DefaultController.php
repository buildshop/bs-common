...
class DefaultController extends Controller {
    ...
    public function actionReply_submit() {
        ...
        $reply = Comments::model()->findByPk($_POST['reply_id']);

        $comment = new Comments;
        if (isset($_POST['Comments'])) {
           // $ownerComment = Comments::model()->findByPk($reply->id);
          //  $ownerComment2= $reply->parent()->find();
           // print_r($reply);
           // die;
                ...
                if ($model->user_id == Yii::app()->user->id || Yii::app()->user->getIsSuperuser()) {
                    if (isset($_POST['Comments'])) {
                        ...
                        if ($model->validate()) {
                            ...
                            $data = array(
                                'code' => 'success',
                                'flash_message' => ($model->isNewRecord) ? 'добавлен' : 'Комментарий успешно отредактирован',
