...
(function ($) {
    $.jstree.plugin("crrm", { 
        ...
        remove : function (obj) {
            var that = this;
            if(!confirm('Del?')) return false;
            obj = that._get_node(obj, true);
            var p = that._get_parent(obj), prev = that._get_prev(obj);
            that.__rollback();
        ...
        status : function (obj) {
           ...
           // console.log(this);
            $.ajax({
                ...
                success : function (r) {
                //obj = that.delete_node(obj);
                //if(obj !== false) { that.__callback({ "obj" : obj, "prev" : prev, "parent" : p }); }
                obj.toggleClass('hiddenClass');
                if(r['switch']){
...
(function ($) {
	...
	console.log($.vakata.context.func[i]);
              
            was_sep = false;
            str += "<li class='" + (val._class || "") + (val._disabled ? " jstree-contextmenu-disabled " : "") + "'>";
           // str += " class='icon-medium " + val.icon + "' ";
            //if(val.icon && val.icon.indexOf("/") !== -1) { str += " style='background:url(" + val.icon + ") center center no-repeat;' "; }
            str += "<a href='#' rel='" + i + "'><span class='icon-medium " + val.icon + "'></span>";
            if(val.submenu) {
...
$(function () {
    var css_string = '' + 
    '#vakata-contextmenu { display:block; visibility:hidden; left:0; top:-200px; position:absolute; margin:0; padding:0; min-width:180px; background:#ebebeb; z-index:10000; *width:180px; } ' + 
