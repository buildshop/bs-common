...
class CMS {
    ...
    public static function textReplace($text, $array) {
        $tmpArray = array();
        $tmpArray['%SITE_NAME%'] = Yii::app()->settings->get('core', 'site_name');
    ...
    public static function userLink($user) {
        return '<ul class="nav nav-pills">
            <li class="dropdown-toggle"><a href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' . $user->login . '<b class="caret"></b></a>
    ...
    public static function date($date, $time = 0) {
       ...
       // $timezone = new DateTime($date);
       // $timezone->setTimezone(new DateTimeZone(Yii::app()->controller->timezone));
        $conf = Yii::app()->settings->get('core', 'format_date');
        $formatted = strtotime($date);
        
       // $timezone->setTimestamp(strtotime(date('H:i',$formatted))); //adjust the object to correct timestamp
        //$q_data_hour= $timezone->format('H:i');

       // $timezone->setTimestamp($formatted);
    ...
    public static function getNowDate($date,$format) {
